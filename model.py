...
def sgd_step(param: NDArray, grad: NDArray, lr: float):
    '''return the SGD-updated parameter array (param - lr * grad).'''
...
def adam_update_v(v: NDArray, grad: NDArray, beta_two: float):
    '''return the updated Adam second moment estimate as an EMA of squared gradients.'''
...
def init_conv_layer(out_channels: int, in_channels: int, kernel_size: int, seed=0):
...
def init_linear_layer(in_features:int, out_features:int, seed=0):
    '''return {'W': He-init matrix (in_features, out_features), 'b': zero bias (out_features,)}'''
